...
document.getElementById('searchBtn').addEventListener('click', function() {
  ...
  fetch('locations.json')
    ...
    .then(function(locations) {
      ...
      // Case 1: Only destination — fly there
      if (!startQuery && destLocation) {
        flyToLocation(destLocation);
        return;
      }

      // Case 2: Start typed, no destination — fly to start
      if (startLocation && !destQuery) {
        flyToLocation(startLocation);
        return;
      }

      // Case 3: Both filled — route between two locations
      ...
      // Case 4: Start empty, destination filled — route from GPS
      if (!startQuery && destLocation) {
        if (!userLatLng) {
...
function flyToLocation(location) {
  ...
  // Remove existing route and stop tracking if a new location is searched
  if (routeControl) {
...
document.getElementById('routeBtn').addEventListener('click', function() {
  ...
  // If a marker was clicked, route to it directly
...
function drawRoute(start, end) {
  ...
  routeControl.on('routesfound', function(e) {
    ...
    // Add collapse toggle to directions panel
setTimeout(function() {
  ...
  // Show cancel button when route is active
...
// Continuously track user location while route is active
...
// Add get directions button in chat
...
function processQuery(query) {
  ...
  map.flyTo([found.lat, found.lng], 18);
        ...
        // Stricter fuzzy match - at least 5 characters must match
       var fuzzy = null;
